...
int64_t Instream::read(isize idx) {
	advLen = 1;//步过字节码
	return read(idx + 1,
	            bytecodeInfo[byteStream->get(idx)].fopLen[0]);
}
...
string Instream::dump(isize idx) {
	...
	switch (bytecodeInfo[in].opNum) { //根据参数个数分情况设定
		case 0://无参字节码
			advLen = 1;
			fmt = "%-5u: %s";
			break;
		...
		case 2://双参字节码
			...
			args[1] = read(idx + advLen, bytecodeInfo[in].fopLen[1]);//读第二个参数
			break;
		default:
			unreachableBranch();
			break;
	}
	sprintf(buffer, fmt, idx, bytecodeInfo[in].strName, args[0], args[1]);
